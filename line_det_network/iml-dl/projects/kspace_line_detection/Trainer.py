...
class PTrainer(Trainer):
    def __init__(self, training_params, model, data, device, log_wandb=True):
        ...
        if pre_trained_weights_path and os.path.exists(pre_trained_weights_path):
            self.model.load_state_dict(torch.load(pre_trained_weights_path, map_location=self.device))
    ...
    def evaluate(self):
        ...
        for data in self.test_ds:
            ...
            with torch.no_grad():
                ...
                metrics['BCE'].append(loss_bce.item())
        ...
        for metric_key, metric_values in metrics.items():
            wandb.log({f"Evaluation/{metric_key}": np.mean(metric_values)})
